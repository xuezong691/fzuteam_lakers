    ...
    try:
        ...
        if connection.is_connected():
            ...
            try:
                ...
                return {"status": "error", "message": "时间格式不正确，请使用 YYYY-MM-DD 格式"}

            # 必填字段校验（Pydantic 已保证非空，但可再加强）
            if not task.place.strip():
                return {"status": "error", "message": "地点不能为空"}
            if not task.staff.strip():
                return {"status": "error", "message": "负责人不能为空"}
            if not task.something.strip():
            ...
            # user_id 已由 Pydantic 强制为 int，但仍可做合理性检查（如 > 0）
            if task.user_id <= 0:
                return {"status": "error", "message": "user_id 必须是大于 0 的整数"}
            ...
            cursor.execute(
                insert_query,
                (task.time_input, task.place, task.staff, task.something, urgency_int, task.user_id)
            )
            connection.commit()
            return {
                "status": "success",
                "message": f"成功添加待办事项，记录编号为: {cursor.lastrowid}",
